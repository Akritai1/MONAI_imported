...
class SwinUNETR(nn.Module):
    ...
    def _check_input_size(self, spatial_shape):
        # Previously guarded by `if not torch.jit.is_scripting()` and used numpy;
        # the TorchScript removal made this run on every forward call, so it uses
        # plain Python arithmetic to avoid per-call tensor allocation (spatial_shape
        # elements are already ints from x_in.shape[2:]).
